...
class VideosController extends Controller
{
    ...
    {
        $video = new Video();
        $form = $this->createForm('AppBundle\Form\VideosType', $video);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('videos_show', array('idVideo' => $video->getIdvideo()));
        }
     ...
     *
     * @Route("/{idVideo}", name="videos_show")
     * @Method("GET")
     */
     ...
     *
     * @Route("/{idVideo}/edit", name="videos_edit")
     * @Method({"GET", "POST"})
     */
    public function editAction(Request $request, Videos $video)
    {
        $deleteForm = $this->createDeleteForm($video);
        $editForm = $this->createForm('AppBundle\Form\VideosType', $video);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('videos_edit', array('idVideo' => $video->getIdvideo()));
        }
     ...
     *
     * @Route("/{idVideo}", name="videos_delete")
     * @Method("DELETE")
     */
    ...
    private function createDeleteForm(Videos $video)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('videos_delete', array('idVideo' => $video->getIdvideo())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
